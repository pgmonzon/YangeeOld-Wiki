...
reemplazar CuentaGastos (mayúscula plural) 4 apariciones
reemplazar CuentaGasto (mayúscula singular) 76 apariciones
reemplazar cuenta_gasto (minúscula singular) 7 apariciones IMPORANTE: no puede tener mayúsculas
...
func CuentaGastoCrear(w http.ResponseWriter, req *http.Request) {
  //-------------------Modificar ###### estas 2 variables
	var documento models.CuentaGasto
  audit := "Crear"
  ...
  // ***********
  //------------------------------------------------------Modificar ######
  estado, valor, mensaje, httpStat, documento, existia := CuentaGastoAlta(documento, req, audit)
  ...
  // ************
  //------------------------------------Modificar ######
  s := []string{"Agregaste ", documento.CuentaGasto}
  //--------------------------------------Modificar ######
...
func CuentaGastoAlta(documentoAlta models.CuentaGasto, req *http.Request, audit string) (string, string, string, int, models.CuentaGasto, bool) {
  //-------------------Modificar ###### las 3 variables
	var documento models.CuentaGasto
  camposVacios := "No podés dejar vacío el campo Cuenta Gasto"
  coll := config.DB_CuentaGasto
  empresaID := context.Get(req, "Empresa_id").(bson.ObjectId)

  // Verifico los campos obligatorios
  // ********************************
  //---------------Modificar ######
  if documentoAlta.CuentaGasto == "" {
  ...
  // ********************
  //-----------------------------------------------------Modificar ######--------------Modificar ######
  estado, valor, mensaje, httpStat, documento, existia := CuentaGastoExiste(documentoAlta.CuentaGasto, req)
...
// Devuelve Estado, Valor, Mensaje, HttpStat, Collection, Existía
func CuentaGastoExiste(documentoExiste string, req *http.Request) (string, string, string, int, models.CuentaGasto, bool) {
  //-------------------Modificar ###### las 3 variables
  var documento models.CuentaGasto
  indice := []string{"empresa_id", "cuenta_gasto"}
  coll := config.DB_CuentaGasto
  empresaID := context.Get(req, "Empresa_id").(bson.ObjectId)
  ...
  // ******************
  //----------------------------------------------Modificar ######
  collection.Find(bson.M{"empresa_id": empresaID, "cuenta_gasto": documentoExiste}).One(&documento)
  // No existe
  if documento.ID == "" {
...
func CuentaGastosTraer(w http.ResponseWriter, req *http.Request) {
  //-------------------Modificar ###### estas 2 variables
  var documento models.CuentaGasto
  var documentos []models.CuentaGasto
  vars := mux.Vars(req)
  orden := vars["orden"]
  limite := vars["limite"]
  ...
  // *****
  //----------------------------------------------Modificar ######
  estado, valor, mensaje, httpStat, documentos := CuentaGastosBuscar(documento, orden, limiteInt, false, "Buscar", req)
  if httpStat != http.StatusOK {
    core.RspMsgJSON(w, req, estado, valor, mensaje, httpStat)
    return
  }
...
func CuentaGastosBuscar(documento models.CuentaGasto, orden string, limiteInt int, borrados bool, audit string, req *http.Request) (string, string, string, int, []models.CuentaGasto) {
  //----------------------Modificar ###### estas 2 variables
  var documentos []models.CuentaGasto
  coll := config.DB_CuentaGasto
  empresaID := context.Get(req, "Empresa_id").(bson.ObjectId)
  ...
  // **************************************
  //-----------Modificar ######
  if orden != "cuenta_gasto" && orden != "-cuenta_gasto" {
  ...
  selector := bson.M{
    ...
    "cuenta_gasto": bson.M{"$regex": bson.RegEx{documento.CuentaGasto, "i"}},
    "borrado": borrados,
  }
  collection := session.DB(config.DB_Name).C(coll)
...
func CuentaGastoTraer(w http.ResponseWriter, req *http.Request) {
  vars := mux.Vars(req)
  ID := vars["docID"]
  ...
  // *****
  //----------------------------------------------Modificar ######
  estado, valor, mensaje, httpStat, documento := CuentaGasto_X_ID(documentoID, "Buscar ID", req)
  if httpStat != http.StatusOK {
    core.RspMsgJSON(w, req, estado, valor, mensaje, httpStat)
    return
  }
...
// Devuelve Estado, Valor, Mensaje, HttpStat, collection
func CuentaGasto_X_ID(documentoID bson.ObjectId, audit string, req *http.Request) (string, string, string, int, models.CuentaGasto) {
  //-------------------Modificar ######
  var documento models.CuentaGasto
  coll := config.DB_CuentaGasto
  empresaID := context.Get(req, "Empresa_id").(bson.ObjectId)
...
func CuentaGastoGuardar(w http.ResponseWriter, req *http.Request) {
  //-------------------Modificar ######
  var documento models.CuentaGasto
  vars := mux.Vars(req)
  ID := vars["docID"]
  ...
  // ***************************************
  //------------------------------------------------------Modificar ######
  estado, valor, mensaje, httpStat, documentoExistente := CuentaGasto_X_ID(documentoID, "Buscar ID", req)
  if httpStat != http.StatusOK {
    core.RspMsgJSON(w, req, estado, valor, mensaje, httpStat)
    return
  }
  ...
  // ********
  //----------------------------------Modificar ######
  estado, valor, mensaje, httpStat = CuentaGastoModificar(documentoID, documento, req, audit)
  if httpStat != http.StatusOK {
    core.RspMsgJSON(w, req, estado, valor, mensaje, httpStat)
    return
  }
  ...
  // ************
  //------------------------------------Modificar ######
  s := []string{"Guardaste ", documento.CuentaGasto}
  //--------------------------------------Modificar ######
  core.RspMsgJSON(w, req, "OK", documento.CuentaGasto, strings.Join(s, ""), http.StatusAccepted)
  return
}

func CuentaGastoHabilitar(w http.ResponseWriter, req *http.Request) {
  //-------------------Modificar ######
  var documento models.CuentaGasto
  vars := mux.Vars(req)
  ID := vars["docID"]
  ...
  //-------Modificar ###### en forma manual
  documento.CuentaGasto = documentoExistente.CuentaGasto
  ...
  // ************
  //------------------------------------Modificar ######
  s := []string{"Habilitaste ", documento.CuentaGasto}
  //--------------------------------------Modificar ######
  core.RspMsgJSON(w, req, "OK", documento.CuentaGasto, strings.Join(s, ""), http.StatusAccepted)
  return
}

func CuentaGastoDeshabilitar(w http.ResponseWriter, req *http.Request) {
  //-------------------Modificar ######
  var documento models.CuentaGasto
  vars := mux.Vars(req)
  ID := vars["docID"]
  ...
  // ************
  //------------------------------------Modificar ######
  s := []string{"Deshabilitaste ", documento.CuentaGasto}
  //--------------------------------------Modificar ######
  core.RspMsgJSON(w, req, "OK", documento.CuentaGasto, strings.Join(s, ""), http.StatusAccepted)
  return
}

func CuentaGastoBorrar(w http.ResponseWriter, req *http.Request) {
  //-------------------Modificar ######
  var documento models.CuentaGasto
  vars := mux.Vars(req)
  ID := vars["docID"]
  ...
  // ************
  //------------------------------------Modificar ######
  s := []string{"Borraste ", documento.CuentaGasto}
  //--------------------------------------Modificar ######
  core.RspMsgJSON(w, req, "OK", documento.CuentaGasto, strings.Join(s, ""), http.StatusAccepted)
  return
}

func CuentaGastoRecuperar(w http.ResponseWriter, req *http.Request) {
  //-------------------Modificar ######
  var documento models.CuentaGasto
  vars := mux.Vars(req)
  ID := vars["docID"]
  ...
  // ************
  //------------------------------------Modificar ######
  s := []string{"Recuperaste ", documento.CuentaGasto}
  //--------------------------------------Modificar ######
  core.RspMsgJSON(w, req, "OK", documento.CuentaGasto, strings.Join(s, ""), http.StatusAccepted)
  return
}
...
// Devuelve Estado, Valor, Mensaje, HttpStat, Collection, Existía
func CuentaGastoModificar(documentoID bson.ObjectId, documentoModi models.CuentaGasto, req *http.Request, audit string) (string, string, string, int) {
  ...
  // ***********************************
  //------------------------------------------------------Modificar ######-------------Modificar ######
  estado, valor, mensaje, httpStat, documentoExiste, _ := CuentaGastoExiste(documentoModi.CuentaGasto, req)
  ...
  updator := bson.M{
    "$set": bson.M{
      "cuenta_gasto": documentoModi.CuentaGasto,
