...
reemplazar SbrSucursales (mayúscula plural) 4 apariciones
reemplazar SbrSucursal (mayúscula singular) 76 apariciones
reemplazar sucursal (minúscula singular) 7 apariciones IMPORANTE: no puede tener mayúsculas
...
func SbrSucursalCrear(w http.ResponseWriter, req *http.Request) {
  //-------------------Modificar ###### estas 2 variables
	var documento models.SbrSucursal
  audit := "Crear"
  ...
  // ***********
  //------------------------------------------------------Modificar ######
  estado, valor, mensaje, httpStat, documento, existia := SbrSucursalAlta(documento, req, audit)
  ...
  // ************
  //------------------------------------Modificar ######
  s := []string{"Agregaste ", documento.SbrSucursal}
  //--------------------------------------Modificar ######
  core.RspMsgJSON(w, req, "OK", documento.SbrSucursal, strings.Join(s, ""), http.StatusCreated)
  return
}

// Devuelve Estado, Valor, Mensaje, HttpStat, Collection, Existía
func SbrSucursalAlta(documentoAlta models.SbrSucursal, req *http.Request, audit string) (string, string, string, int, models.SbrSucursal, bool) {
  //-------------------Modificar ###### las 3 variables
	var documento models.SbrSucursal
  camposVacios := "No podés dejar vacío el campo Categoría"
  coll := config.DB_SbrSucursal
  empresaID := context.Get(req, "Empresa_id").(bson.ObjectId)

  // Verifico los campos obligatorios
  // ********************************
  //---------------Modificar ######
  if documentoAlta.SbrSucursal == "" {
  ...
  // ********************
  //-----------------------------------------------------Modificar ######--------------Modificar ######
  estado, valor, mensaje, httpStat, documento, existia := SbrSucursalExiste(documentoAlta.SbrSucursal, req)
...
// Devuelve Estado, Valor, Mensaje, HttpStat, Collection, Existía
func SbrSucursalExiste(documentoExiste string, req *http.Request) (string, string, string, int, models.SbrSucursal, bool) {
  //-------------------Modificar ###### las 3 variables
  var documento models.SbrSucursal
  indice := []string{"empresa_id", "sucursal"}
  coll := config.DB_SbrSucursal
  empresaID := context.Get(req, "Empresa_id").(bson.ObjectId)
  ...
  // ******************
  //----------------------------------------------Modificar ######
  collection.Find(bson.M{"empresa_id": empresaID, "sucursal": documentoExiste}).One(&documento)
  // No existe
  if documento.ID == "" {
...
func SbrSucursalesTraer(w http.ResponseWriter, req *http.Request) {
  //-------------------Modificar ###### estas 2 variables
  var documento models.SbrSucursal
  var documentos []models.SbrSucursal
  vars := mux.Vars(req)
  orden := vars["orden"]
  limite := vars["limite"]
  ...
  // *****
  //----------------------------------------------Modificar ######
  estado, valor, mensaje, httpStat, documentos := SbrSucursalesBuscar(documento, orden, limiteInt, false, "Buscar", req)
  if httpStat != http.StatusOK {
    core.RspMsgJSON(w, req, estado, valor, mensaje, httpStat)
    return
  }
...
func SbrSucursalesBuscar(documento models.SbrSucursal, orden string, limiteInt int, borrados bool, audit string, req *http.Request) (string, string, string, int, []models.SbrSucursal) {
  //----------------------Modificar ###### estas 2 variables
  var documentos []models.SbrSucursal
  coll := config.DB_SbrSucursal
  empresaID := context.Get(req, "Empresa_id").(bson.ObjectId)
  ...
  // **************************************
  //-----------Modificar ######
  if orden != "sucursal" && orden != "-sucursal" {
  ...
  selector := bson.M{
    ...
    "sucursal": bson.M{"$regex": bson.RegEx{documento.SbrSucursal, "i"}},
    "borrado": borrados,
  }
  collection := session.DB(config.DB_Name).C(coll)
...
func SbrSucursalTraer(w http.ResponseWriter, req *http.Request) {
  vars := mux.Vars(req)
  ID := vars["docID"]
  ...
  // *****
  //----------------------------------------------Modificar ######
  estado, valor, mensaje, httpStat, documento := SbrSucursal_X_ID(documentoID, "Buscar ID", req)
  if httpStat != http.StatusOK {
    core.RspMsgJSON(w, req, estado, valor, mensaje, httpStat)
    return
  }
...
// Devuelve Estado, Valor, Mensaje, HttpStat, collection
func SbrSucursal_X_ID(documentoID bson.ObjectId, audit string, req *http.Request) (string, string, string, int, models.SbrSucursal) {
  //-------------------Modificar ######
  var documento models.SbrSucursal
  coll := config.DB_SbrSucursal
  empresaID := context.Get(req, "Empresa_id").(bson.ObjectId)
...
func SbrSucursalGuardar(w http.ResponseWriter, req *http.Request) {
  //-------------------Modificar ######
  var documento models.SbrSucursal
  vars := mux.Vars(req)
  ID := vars["docID"]
  ...
  // ***************************************
  //------------------------------------------------------Modificar ######
  estado, valor, mensaje, httpStat, documentoExistente := SbrSucursal_X_ID(documentoID, "Buscar ID", req)
  if httpStat != http.StatusOK {
    core.RspMsgJSON(w, req, estado, valor, mensaje, httpStat)
    return
  }
  ...
  // ********
  //----------------------------------Modificar ######
  estado, valor, mensaje, httpStat = SbrSucursalModificar(documentoID, documento, req, audit)
  if httpStat != http.StatusOK {
    core.RspMsgJSON(w, req, estado, valor, mensaje, httpStat)
    return
  }
  ...
  // ************
  //------------------------------------Modificar ######
  s := []string{"Guardaste ", documento.SbrSucursal}
  //--------------------------------------Modificar ######
  core.RspMsgJSON(w, req, "OK", documento.SbrSucursal, strings.Join(s, ""), http.StatusAccepted)
  return
}

func SbrSucursalHabilitar(w http.ResponseWriter, req *http.Request) {
  //-------------------Modificar ######
  var documento models.SbrSucursal
  vars := mux.Vars(req)
  ID := vars["docID"]
  ...
  //-------Modificar ###### en forma manual
  documento.SbrSucursal = documentoExistente.SbrSucursal
  ...
  // ************
  //------------------------------------Modificar ######
  s := []string{"Habilitaste ", documento.SbrSucursal}
  //--------------------------------------Modificar ######
  core.RspMsgJSON(w, req, "OK", documento.SbrSucursal, strings.Join(s, ""), http.StatusAccepted)
  return
}

func SbrSucursalDeshabilitar(w http.ResponseWriter, req *http.Request) {
  //-------------------Modificar ######
  var documento models.SbrSucursal
  vars := mux.Vars(req)
  ID := vars["docID"]
  ...
  // ************
  //------------------------------------Modificar ######
  s := []string{"Deshabilitaste ", documento.SbrSucursal}
  //--------------------------------------Modificar ######
  core.RspMsgJSON(w, req, "OK", documento.SbrSucursal, strings.Join(s, ""), http.StatusAccepted)
  return
}

func SbrSucursalBorrar(w http.ResponseWriter, req *http.Request) {
  //-------------------Modificar ######
  var documento models.SbrSucursal
  vars := mux.Vars(req)
  ID := vars["docID"]
  ...
  // ************
  //------------------------------------Modificar ######
  s := []string{"Borraste ", documento.SbrSucursal}
  //--------------------------------------Modificar ######
  core.RspMsgJSON(w, req, "OK", documento.SbrSucursal, strings.Join(s, ""), http.StatusAccepted)
  return
}

func SbrSucursalRecuperar(w http.ResponseWriter, req *http.Request) {
  //-------------------Modificar ######
  var documento models.SbrSucursal
  vars := mux.Vars(req)
  ID := vars["docID"]
  ...
  // ************
  //------------------------------------Modificar ######
  s := []string{"Recuperaste ", documento.SbrSucursal}
  //--------------------------------------Modificar ######
  core.RspMsgJSON(w, req, "OK", documento.SbrSucursal, strings.Join(s, ""), http.StatusAccepted)
  return
}
...
// Devuelve Estado, Valor, Mensaje, HttpStat, Collection, Existía
func SbrSucursalModificar(documentoID bson.ObjectId, documentoModi models.SbrSucursal, req *http.Request, audit string) (string, string, string, int) {
  ...
  // ***********************************
  //------------------------------------------------------Modificar ######-------------Modificar ######
  estado, valor, mensaje, httpStat, documentoExiste, _ := SbrSucursalExiste(documentoModi.SbrSucursal, req)
  ...
  updator := bson.M{
    "$set": bson.M{
      "sucursal": documentoModi.SbrSucursal,
